            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex flex-col lg:items-center gap-3">
                ...
                <div className="relative">
                  ...
                  <ChevronDown className="absolute top-1/2 right-2.5 -translate-y-1/2 w-3.5 h-3.5 text-app-text-light pointer-events-none" />
                </div>
              </div>
            </div>
            ...
            {totalPages > 1 && (
              <div className="flex-center gap-2 mt-16">
                {Array.from({length: totalPages}).map((_, i) => (
                  <button key={i} onClick={() => {updateFilters('page', String(i + 1)); window.scrollTo(0, 0);}} className={`size-9 rounded-lg text-sm font-medium transition-colors ${page === i + 1 ? "bg-app-green text-white" : "bg-white text-app-text-light hover:bg-app-cream"}`}>{i + 1}</button>
                ))}
              </div>
            )}
          </main>
